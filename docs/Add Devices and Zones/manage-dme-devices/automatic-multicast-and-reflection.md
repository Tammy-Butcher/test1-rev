...
Automatic multicast and reflection _greatly_ expands the reach of your Cloud-sourced events and Webcasts. This capability includes all RTMP ingest, WebEx Live, and all [Vbrick video conference integrations](doc:video-conference-vc-integrations).
...
> 📘 Note
> 
> If both **Auto Multicast for Cloud Streams** and **Auto Unicast for Cloud Streams** are configured, then viewers/browsers within the Zone who cannot take advantage of multicast will fall back and receive a _reflected_ unicast MBR stream from the DME.

## Initial Multicast Configuration

_Before_ you configure automatic multicast and/or reflection for your DMEs in Rev:
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/edb0618-enableAutomaticMulticast.png",
        "enableAutomaticMulticast.png",
        1133
      ],
      "align": "center",
      "caption": "You must have the correct DME and VBM software builds installed before this setting is visible in a DME Device"
    }
  ]
}
[/block]
...
3. In the second octet, either use numbers from 1–127, or 129–255, do _not_ mix ranges on a given network.
...
> 🚧 Important!
> 
> There are some HLS streams and tags that are _not_ supported.  View the DME Help topic, **Rev Initiated Multicast and Reflection**.
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/6539e6e-customDevicesTab.png",
        "customDevicesTab.png",
        802
      ],
      "align": "center",
      "caption": "The Custom Devices tab is available on DME Devices that have DME v3.23+ installed"
    }
  ]
}
[/block]
...
[block:parameters]
{
  "data": {
    "h-0": "Setting",
    "h-1": "Description",
    "0-0": "Select Custom Device",
    "0-1": "**Custom Devices** already created appear in the dropdown. Only HLS streams can be used for this feature so the Custom Device must also have one or more HLS streams.  \n  \n**View**: [Add a Custom Device](doc:add-a-source-or-custom-device#add-a-custom-device)",
    "1-0": "Select Stream Name",
    "1-1": "Each stream has a unique name (when you created the Custom Device). Here you select the appropriate name for the URL stream you want to use for multicast and/or reflection. Custom Devices can contain multiple HLS streams.  \n  \nNote that you may _not_ use the same name more than once.",
    "2-0": "Enable DME Reflection",
    "2-1": "Select this checkbox if you want the DME to reflect the stream(s). Clicking this creates a new stream in the DME (the name is listed within the user interface.)  \n  \nThis stream _must_ be included within the **Presentation Profile** (with the DME as the destination) to generate automatic reflection through the DME.  \n  \nIncluding the Custom Device as a destination within the Presentation Profile uses the stream(s) as it does today – unicast.  \n  \n**Note**: At least one of the **Enable DME Reflection or Enable Automatic Multicast** checkboxes _must_ be selected.",
    "3-0": "Enable Automatic Multicast during webcast",
    "3-1": "Select this checkbox to enable automatic multicasting during Webcasts.  Clicking this creates a new stream in the DME (the name is listed within the user interface.)  \n  \nThis stream _must_ be included within the **Presentation Profile** (with the DME as the destination) to generate automatic multicast from this DME.  \n  \nIncluding the Custom Device as a Destination within the Presentation Profile will use the stream(s) as it does today – unicast.  \n  \n**Note**: At least one of the **Enable DME Reflection or Enable Automatic Multicast** checkboxes _must_ be selected."
  },
  "cols": 2,
  "rows": 4,
  "align": [
...
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/4f395d9-autoMulticastStreamedZones.png",
        "autoMulticastStreamedZones.png",
        722
      ],
      "align": "center",
      "caption": "Automatic Multicast streams are noted by a distinct icon in Rev Zones"
    }
  ]
}
[/block]