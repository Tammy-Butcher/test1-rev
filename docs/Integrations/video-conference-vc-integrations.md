...
* It must be used throughout the event. In other words, you may not use a video conference as the source for half the event and then switch to a “traditional” source (camera/presentation profile) for the second half of the event. You *must* use one or the other source per event or an error occurs.
