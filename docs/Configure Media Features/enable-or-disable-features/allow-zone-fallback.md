...
Select the **Allow zone fallback for VOD streams** checkbox under **Media Settings** > **Features** to enable. This option is enabled by default. 
...
[block:callout]
{
  "type": "danger",
  "title": "Warning!",
  "body": "If this option is disabled, zone logic only returns the playback URL of the DMEs in the user’s matched zone and zone logic does not return a playback URL from Rev.\n\nFurther, if there is **no **matching URL, then the viewer is displayed the following message: **“Playback of this video is not available at this time. Please try again later.”** "
}
[/block]